...
// Write a prototype method for the User constructor function named removeSavedPost 
//that will take in one number parameter representing the post id. Use this id to find and remove the matching 
//object in the savedPosts array.

User.prototype.removeSavedPost = function(id) {
  let newArr = this.savedPosts.filter(elem=> elem.id !== id)
  
  this.savedPosts = [...newArr]
}
////////// PROBLEM 7 //////////
...
//will take in two number parameters. The first will be an id (a number) and the second will 
...
User.prototype.changePostRating = function(id,newRating) {
  let arr = this.savedPosts.filter(elem => elem.id === id)
 let index = this.savedPosts.indexOf(arr[0])
